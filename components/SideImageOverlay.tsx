...
const SideImageWithOverlay = ({ imageSource, title, smallerTitle, text, onPress }: SideImageOverlayProps) => {
	return (
		<TouchableOpacity style={[LARGE_CONTAINER_STYLING.SMALL_ROUNDED_iMAGE_CONTAINER_STYLING]} onPress={onPress}>
			<Image source={imageSource} style={LARGE_CONTAINER_STYLING.IMAGE_IN_SMALL_CONTAINER_STYLING_FARMERSPOINT} />
			<View>
				<View style={{ flexDirection: "row", alignItems: "center" }}>
					<Text style={[FONTS.LARGE_TITLE, { marginTop: -75, marginLeft: 90, fontSize: 22 }]}>{title}</Text>
					{smallerTitle && (
						<Text style={[FONTS.SNOW_REGULAR_FONT_TWO, { marginTop: -65, marginLeft: 3, fontSize: 10 }]}>
							{smallerTitle}
						</Text>
					)}
				</View>
